...
class conclusion:
    ...
    def __init__(self, cusTable, settings):
        ...
        if "title" in allKeys:
            ...
            self.title = settings['conclusion']['title']

        # Use Previous Results for This Section
        if "usePrevResults" in allKeys:
            self.usePrevResults = cusTable['usePrevResults']
        else:
            self.usePrevResults = settings['conclusion']['usePrevResults']

        # System Prompts are Added (NOT REPLACED)!
        if "systemPrompts" in allKeys:
            self.systemPrompts = cusTable['systemPrompts'] + settings['summary']['systemPrompts']
        else:
            self.systemPrompts = settings['conclusion']['systemPrompts']
        # User Prompts are Added (NOT REPLACED)!
        if "userPrompts" in allKeys:
            self.userPrompts = cusTable['userPrompts'] + settings['conclusion']['userPrompts']
        else:
            self.userPrompts = settings['conclusion']['userPrompts']
    ...
    # Generating The Text Prompt for The Table!
    def generateConclusionPrompts(self, generationBlocks):
    ...
    def ConcludeGPT(self, client):

        if self.apiPrompt == []:
            print("The System Prompt for CHAT-GPT is Empty, Section: Conclusion")
            print("CHECK ConcludeGPT function of Summary")
            print("Program Terminating!!!")
            sys.exit()

        chat_completion = client.chat.completions.create(messages=self.apiPrompt, model=self.gptModel)
        self.chatGPTResponse = chat_completion.choices[0].message.content
        print("Summary Section Completed with the following Conclusion ", self.chatGPTResponse)
        return self.chatGPTResponse
